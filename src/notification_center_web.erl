...
%% @doc Web server for notification_center.

-module(notification_center_web).
-author("Mochi Media <dev@mochimedia.com>").
        ...
        {match,[MatchList]} -> 
            ...
            case length(MatchList) of
                0 -> 
                    ...
                    notification_center_views:Function(Method, Req);
                Length when Length > 0 -> 

                    % We pass URL parameters we captured to the function
                    Args = lists:append([[Method, Req], MatchList]),
                    %% Unknown number of arguments thats why we use apply/3
                    apply(notification_center_web,Function, Args)
            end;
        _ -> 
...
loop(Req, DocRoot) ->
    "/" ++ Path = Req:get(path),
    try
        case dispatch(Req, notification_center_views:urls()) of
            none -> 
